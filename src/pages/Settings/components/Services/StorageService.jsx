...
export default function StorageService(props) {
  ...
  const updateStorageService = (index, field, value) => {
    ...
    const { type, gbs, duration } = updatedServices[index];
    let monthlyPrice = 0;

    monthlyPrice = UNIT_COSTS[type] * gbs * duration;
  ...
  const calculateTotalMonthlyPrice = () => {
    return props.storageService.reduce((sum, row) => {
      const gbs = parseFloat(row.gbs) || 0; // Ensure monthlyPrice is a number
      const duration = parseFloat(row.duration) || 0; // Ensure monthlyPrice is a number
      const price = UNIT_COSTS[row.type] * gbs * duration; // Ensure monthlyPrice is a number
      return sum + price;
  ...
  return (
    <React.Fragment>
      ...
      <Row style={{ fontSize: '14px', fontWeight: 'bold' }}>
        ...
        <Col lg={1}> <span>Size (GB)</span> </Col>
        ...
        <Row key={index} style={{ fontSize: '14px', fontWeight: 'bold', marginBottom: '10px' }}>
          ...
          <Col lg={2}>
            <Form.Group className="mb-3" controlId={`fgType-${index}`}>
              ...
              >
                {row.serviceName !== '' ? (<option>Select Type</option>) : (<option>Please First Select Service</option>)}
                {row.serviceName === 'elastic' && (
                  <>
                    <option value="SSD">General Purpose SSD (Per GB)</option>
                    <option value="HDD">HDD (Per GB)</option>
                  </>
                )}
                {row.serviceName === 'obs' && <option value="OBS">OBS for Object Data or IMS (Images)</option>}
                {row.serviceName === 'bv' && <option value="Backup">Licence + OBS</option>}
              </Form.Select>
            </Form.Group>
          </Col>
          <Col lg={1}>
          ...
          <Col lg={2}>
            <Form.Group className="mb-3" controlId={`fgPrice-${index}`}>
              <Form.Control
                ...
                disabled={true}
                placeholder="Price"
                value={(UNIT_COSTS[row.type] * parseFloat(row.gbs) * parseFloat(row.duration)).toFixed(2)}
                readOnly
                style={{ fontSize: '16px' }}
              />
            </Form.Group>
      ...
      <Row>
        <Col>
          <Button variant="outline-primary" onClick={addStorageServiceRow} style={{ marginRight: '5px' }} >
            Add
          </Button>
        </Col>
